    ...
    def create_query_log(self, log_data: QueryLogCreate) -> bool:
        ...
        try:
            ...
            with conn.cursor() as cursor:
                sql = """
                      INSERT INTO query_logs (user_id, user_role, query_type, page_name, query_content, cypher_query, \
...
async def search_nodes(
    ...
    try:
        with db_service.get_neo4j_session() as session:
            ...
            return {"nodes": nodes, "count": len(nodes), "search_text": search_request.text,
    ...
    try:
        ...
        with db_service.get_neo4j_session() as session:
            ...
            has_relationships = any(
                'type' in record and 'start_node_id' in record for record in records if isinstance(record, dict))
        ...
        with conn.cursor() as cursor:
            for prop in properties:
                sql = """
                      ...
                      VALUES (description), data_type = \
                      VALUES (data_type), sort_order = \
                      VALUES (sort_oørder), is_display = \
                      VALUES (is_display) \
                      """
                cursor.execute(sql, (
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("neo4j_backend_yuhe:app", host="0.0.0.0", port=8001, reload=False)